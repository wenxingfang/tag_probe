...
//  TH1F *hhlt2E1 = (TH1F*) hhlt_base->Clone("hhlt2E1");
  ...
  chain->SetBranchAddress("mc_phi", &mc_phi) ;
    ...
    for(int j=0; j<gsf_n;j++){
	 for(int k=j+1;k<gsf_n;k++){
	 if((gsf_charge->at(j)+gsf_charge->at(k)) !=0) continue;
	 ...
	 if(HEEP_cutflow51_Et_value->at(k) < 35 || abs(HEEP_cutflow51_eta_value->at(k))>2.5 || ( abs(HEEP_cutflow51_eta_value->at(k))>1.442 && abs(HEEP_cutflow51_eta_value->at(k)) <1.56) ) continue;
	 ...
	 if( abs(HEEP_cutflow51_eta_value->at(j))>1.56 && abs(HEEP_cutflow51_eta_value->at(j))<2.5 ){
       ...
       if( abs(HEEP_cutflow51_eta_value->at(k)) < 1.442){
	    if( HEEP_cutflow51_EcalDriven_value->at(k)==1 && HEEP_cutflow51_dEtaIn_value->at(k)<0.004 && HEEP_cutflow51_dPhiIn_value->at(k)< 0.06 && HEEP_cutflow51_HOverE_value->at(k)< (0.5+2.5/gsf_energy->at(k)) && ( HEEP_cutflow51_E1x5OverE5x5_value->at(k)> 0.83 || HEEP_cutflow51_E2x5OverE5x5_value->at(k)> 0.94 ) && HEEP_cutflow51_missingHits_value->at(k)< 2 && HEEP_cutflow51_dxyFirstPV_value->at(k)<0.02 ) ID_k=1;
          if(HEEP_cutflow51_IsolPtTrks_value->at(k)< 5) {if( HEEP_cutflow51_isolEMHadDepth1_value->at(k) < (2+0.03*HEEP_cutflow51_Et_value->at(k)+0.28*ev_fixedGridRhoAll) ) Iso_k=1;}				   				                                                       }

       if( abs(HEEP_cutflow51_eta_value->at(k))>1.56 && abs(HEEP_cutflow51_eta_value->at(k))<2.5 ){
          if( HEEP_cutflow51_EcalDriven_value->at(k)==1 && HEEP_cutflow51_dEtaIn_value->at(k)<0.006 && HEEP_cutflow51_dPhiIn_value->at(k)< 0.06 && HEEP_cutflow51_HOverE_value->at(k)< (0.5+12.5/gsf_energy->at(k)) && HEEP_cutflow51_SigmaIetaIeta_value->at(k)< 0.03 && HEEP_cutflow51_missingHits_value->at(k)< 2 && HEEP_cutflow51_dxyFirstPV_value->at(k)<0.05 ) ID_k=1;
          if(HEEP_cutflow51_IsolPtTrks_value->at(k)< 5){if( ( HEEP_cutflow51_Et_value->at(k)<50 && HEEP_cutflow51_isolEMHadDepth1_value->at(k)<(2.5+0.28*ev_fixedGridRhoAll) ) || (HEEP_cutflow51_Et_value->at(k)>50 && HEEP_cutflow51_isolEMHadDepth1_value->at(k)<(2.5+0.28*ev_fixedGridRhoAll+0.03*(HEEP_cutflow51_isolEMHadDepth1_value->at(k)-50)) ) ) Iso_k=1;} 
							                                                                                                     }


	                         if( (ID_j+Iso_j)==2 || (ID_k+Iso_k)==2 ){ 
                               gsf_1.SetPtEtaPhiM(gsf_pt->at(j),gsf_eta->at(j),gsf_phi->at(j),0.000511);
		         		 gsf_2.SetPtEtaPhiM(gsf_pt->at(k),gsf_eta->at(k),gsf_phi->at(k),0.000511);
					 Z=gsf_1+gsf_2;
					 if(Z.M()>max) {max=Z.M();}
...
*/
   ...
   TFile *f1=new TFile("Tag_probe_highestZ_0630.root","UPDATE");
